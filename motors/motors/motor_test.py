...
import time
import traceback
from . import rhino_params
from . import motor_driver
state = 1
driver_left = motor_driver.Controller("/dev/ttyUSB0", 7)
driver_right = motor_driver.Controller("/dev/ttyUSB1", 7)
...
def main():
    ...
    try:
        while True:
            if state == 1:
                encoder_ticks_left_start = driver_left.get_position_32bit()
                encoder_ticks_right_start = driver_right.get_position_32bit()
                ...
                encoder_ticks_left = driver_left.get_position_32bit()
                encoder_ticks_right = driver_right.get_position_32bit()
                ...
                '''
                if (abs(encoder_ticks_left_start - encoder_ticks_left) >= rhino_params.ROTATION_360_ENCODER_COUNT) or \
                        (abs(encoder_ticks_right_start - encoder_ticks_right) >= rhino_params.ROTATION_360_ENCODER_COUNT):
                    state = 6
                '''
            elif state == 3:
